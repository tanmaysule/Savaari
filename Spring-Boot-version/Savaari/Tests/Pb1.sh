...
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=201&sourceLoc=2&destinationLoc=10")
echo $rideDetails
...
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=202&sourceLoc=1&destinationLoc=11")
rideId=$(echo $rideDetails | cut -d' ' -f 1)
if [ "$rideId" != "-1" ];
then
