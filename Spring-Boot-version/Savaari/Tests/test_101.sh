...
# cust 201 requests ride (should be accepted)
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=201&sourceLoc=10&destinationLoc=20")
...
#========================================================================================================================================
# cust 202 requests ride (should be rejected)
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=202&sourceLoc=10&destinationLoc=20")
rideId=$(echo $rideDetails | cut -d' ' -f 1)
if [ ! "$rideId" = "-1" ];
then
