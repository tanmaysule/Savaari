...
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=201&sourceLoc=14&destinationLoc=10")
...
rideDetails=$(curl -s "http://localhost:8081/requestRide?custId=201&sourceLoc=10&destinationLoc=14")
rideId=$(echo $rideDetails | cut -d' ' -f 1)
if [ ! "$rideId" = "-1" ];
then
	echo "Customer 201 alloted a ride number " $rideId
